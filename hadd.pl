...
if ($all) {
	...
	@periods = ('before'   => {first => '12172024', second => '12175062', list => ''},
			'after'    => {first => '12175086', second => '12179097', list => ''},
			);
}
while (@ARGV) {
	$_ = shift @ARGV;
...
#    my $cmd = "test -r $rootfile || hadd -T -f $rootfile $list";
		my $cmd = "test -r $rootfile || hadd -f -T $rootfile $list";
		$cmd .= ">&  $log";
