...
# Defaults arguments
iarg=0
args=('0' 'joblist.list' 'outlist.list' 'submit_locallog.xml' 'pico.list') # submitjobs_flag All_job_list finished_job_list submit_xml all_submit_filelist
for i in $@
do 
    args[$iarg]=$i
    let "iarg+=1"
...
do 
    ...
    if [ $residue -eq 0 ]
    then 
	percent=$(echo "scale=3;${count} / ${num_all} * 100" | bc) # "scale" used the set display precision. 
...
LOG_INFO " Fished File number  : $count "
...
#===============================
#Version: 1.4 
#Time: Thu Dec  3 11:38:19 PST 2015 
#Author: Long Zhou 
