...
Example:
   ./ReRun_sch.sh runmode List_dir out_dir "submit_xml" "submit_file_list"
...
# Defaults arguments
iarg=0
args=('0' 'list' 'out' 'submit_local.xml' 'pico.list') # submitjobs_flag All_job_list finished_job_list submit_xml all_submit_filelist
for i in $@
do 
    args[$iarg]=$i
    let "iarg+=1"
...
issubmit=${args[0]}
dir_file_all=${args[1]}
dir_file_part=${args[2]}
submitxml=${args[3]}
submitlist=${args[4]}

# Set output and rerun xml and initialize lists
file_all="${timestamp}_all.list"
file_part="${timestamp}_par.list"
rerunxml="re_$submitxml"
outname="${timestamp}.list"
finishedlist="${timestamp}_finished.list"

find $dir_file_all -name "*.list" > $file_all
find $dir_file_part -name "*.root" > $file_part
...
rerun_dir="ReRun/${timestamp}"
mkdir -p $rerun_dir
cp $outname ReRun/
mv $outname $rerun_dir
mv $finishedlist $rerun_dir
mv $file_all $rerun_dir
mv $file_part $rerun_dir
